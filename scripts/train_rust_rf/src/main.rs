...
/// Tests random forest classifier on test set and measures inference times
fn test_rf(
    clf: &RandomForestClassifier<f64, usize, DenseMatrix<f64>, Vec<usize>>,
    ...
    let (y_hat, y_time) = predict_and_measure_rf(clf, &x_test);
    println!(
...
fn main() -> Result<()> {
    ...
    let param_n_trees = args.param_n_trees;
    ...
    let mut params = RandomForestClassifierParameters::default().with_max_depth(param_max_depth).with_n_trees(param_n_trees);
    params.seed = 0;

    // Train and test
    let clf = train_rf(train_dataset, params)?;
    let model_pred = test_rf(&clf, test_dataset);
