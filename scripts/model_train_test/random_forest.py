...
def train_python_rf(X, y):
    ...
    """
    print(utils.CYAN + "Training Python RF" + utils.RESET)
    ...
    for _fold, (train_idx, test_idx) in enumerate(cv.split(X, y)):
        ...
        tuned = tune_hyperparams_rf(X_train, y_train)
...
def test_python_rf(clf, X_test, y_test, num_trials=1000):
    """
    ...
    """
    print(utils.CYAN + "Testing Python RF" + utils.RESET)
...
def train_test_rust_rf(dataset_dir, feature_set, params):
    """
    ...
    """
    print(utils.CYAN + "Training Rust RF" + utils.RESET)
    ...
    if not os.path.exists(model_dir):
        os.mkdir(model_dir)
    model_pred_json = os.path.join(model_dir, "rust_rf_pred.json")
    model_bin = os.path.join(model_dir, "rust_rf.bin")

    max_depth = params["max_depth"]
    n_trees = params["n_estimators"]
    manifest_path = f"{consts.rust_train_dir}/train_rust_rf/Cargo.toml"

    feature_comma = utils.feature_comma(feature_set)

    # Rust RF trains the model using best hyperparameters found by CV in Python, 
    # and outputs its predictions on the test set with inference timings.
    cmd = f"cargo run --manifest-path={manifest_path} --release -- --train-dataset={train_dataset_csv} --test-dataset={test_dataset_csv} --feature-comma={feature_comma} --model-pred={model_pred_json} --model-bin={model_bin} --param-max-depth={max_depth} --param-n-trees={n_trees}"
...
def eval_rust_model(model_pred_json):
    ...
    """
    print(utils.CYAN + "Evaluating Rust RF" + utils.RESET)
